...
static const unint32 CK[32] = {
    ...
    //产生密钥过程
unint32 generate_key(unint32 *a)
    {
    //第一步 将传入的数组首先和ck进行异或得到一个新的32bit的数据
    unint32 first_step = xor_funtion((xor_funtion((xor_funtion(a[1],a[2])),a[3])),CK[0]);
    ...
    return (xor_funtion(xor_funtion(B,B_left),B_right));  //返回最终的结果
    }
int main()
{
    ...
    */
    ...
    unint32 C = generate_key(K_num);
    unint32 k4 = xor_funtion(k0,C);
    printf("%08X",k4);
    return 0;
}